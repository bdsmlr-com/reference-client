...
describe('shared control panel', () => {
  it('defines a shared compact control-panel component with inline separators and optional sections', () => {
    ...
    const sortSrc = readFileSync(join(ROOT, 'components/sort-controls.ts'), 'utf8');
  ...
  it('uses compact popover selectors for shared type and variant filters', () => {
    ...
    expect(variantSrc).toContain('this.pendingExplicitSelection = explicitSelection;');
    expect(variantSrc).toContain('this.lockStateKnown = true;');
    expect(variantSrc).toContain('this.restorePersistedSelection();');
